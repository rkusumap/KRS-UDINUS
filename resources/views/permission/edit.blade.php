    ...
    <div class="section-body">
        ...
        <div class="row">
            <div class="col-12 col-md-12 col-lg-12">
                <div class="card">
                    <form action="{{ route('permission.update',$dataLevel->id_level) }}" id="form" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="card-body">
                            <div class="form-group">
                                <label>Nama Level</label>
                                <input value="{{ $dataLevel->name_level }}" type="text" name="name_level" id="name_level" class="form-control">
                            </div>

                            <div class="form-group">
                                <label>Kode Level</label>
                                <input value="{{ $dataLevel->code_level }}" type="text" name="code_level" id="code_level" class="form-control">
                            </div>
                                ...
                                @foreach ($dataModule as $module)
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-10 input-gmd">
                                            <input type="text" value="{{$module->role($dataLevel->id_level)->first()->action_gmd ?? ''}}" name="action_gmd[{{$module->id_module}}]" class="form-control input-tags" aria-describedby="action_gmd">
                                        </div>
                                    </div>
                                    @if ($module->modules->count() > 0)
                                        @foreach ($module->modules as $mod)
                                            <div class="form-group row ml-3">
                                                ...
                                                <div class="col-sm-10 input-gmd">
                                                    <input type="text" value="{{$mod->role($dataLevel->id_level)->first()->action_gmd ?? ''}}" name="action_gmd[{{$mod->id_module}}]" class="form-control input-tags" aria-describedby="action_gmd">
                                                </div>
                                            </div>
                                        @endforeach
                                    @endif
