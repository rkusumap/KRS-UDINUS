...
class PermissionController extends Controller
{
    ...
    public function index(Request $request) {
        $get_module = get_module_id('permission');
        if (!notAccessBackHome($get_module)) {
            return redirect('/home');
        }
        return view('permission.index',compact('get_module'));
    }
    ...
    public function json()
    {
        ...
        return Datatables::of($datas)
            ->addColumn('action', function ($data) {
                //get module akses
                $id_module = get_module_id('permission');

                //detail
                $btn_detail = '';
                if (isAccess('read', $id_module, auth()->user()->level_user)) {
                    $btn_detail = '<a class="dropdown-item" href="' . route('permission.show', $data->id_level) . '">Detail</a>';
                }

                //edit
                $btn_edit = '';
                if (isAccess('update', $id_module, auth()->user()->level_user)) {
                    $btn_edit = '<button type="button" onclick="location.href=' . "'" . route('permission.edit', $data->id_level) . "'" . ';" class="btn btn-sm btn-info">Edit</button>';
                }
    ...
    public function create(Request $request) {
        ...
        $dataModule = Module::where('induk_module', '0')->orderby('order_module', 'ASC')->get();
        return view('permission.create',compact('dataModule'));
    }
    ...
    public function edit($id,Request $request) {
        ...
        $dataLevel = Level::find($id);
        $dataModule = Module::where('induk_module', '0')->orderby('order_module', 'ASC')->get();
        return view('permission.edit',compact('dataModule','dataLevel'));
    }
    ...
    public function show($id) {
        ...
        $data = Level::find($id);
        $dataModule = Module::where('induk_module', '0')->orderby('order_module', 'ASC')->get();
        return view('permission.show',compact('data','dataModule'));
    }
